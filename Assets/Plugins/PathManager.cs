...
public class PathManager : object
{
    ...
    private void linkUpNodes(Point loc, bool checkNearby)
    {
        ...
        for (var i = 0; i < 18; i++)
        {
            ...
            if (i >= 14)
            {
                ...
                if (IsSolid(loc + newTrans) || IsSolid(loc + newTrans2) || IsSolid(loc + newTrans + G.transList[0]) ||
                    IsSolid(loc + newTrans2 + G.transList[0]))
                    noPath = true;
            }
    ...
    private void checkGroup(Node u)
    {
        ...
        while (nodes.Count != 0)
        {
            n = nodes.Dequeue();
            //stackCheck++;
            for (var i = 0; i < n.linkedNodes.Length; i++)
            {
                w = n.linkedNodes[i];
