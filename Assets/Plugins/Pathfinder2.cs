...
public class Pathfinder2
{
    ...
    {
        var openList = new List<int>(16384);
        var closedNodes = new List<Node>(16384);
        var nodeList = new List<Node>(16384);

        var closedNodesIndex = new List<Node>(16384);
        var openNodesIndex = new List<Node>(16384);
        ...
        while (!(currentNode.x == destinationNode.x && currentNode.y == destinationNode.y &&
                 currentNode.z == destinationNode.z))
        {
            ...
            for (i = 0; i < l; i++)
                if (connectedNodes[i] != null)
                {
                    ...
                    if (Mathf.Abs(testNode.x - currentNode.x) + Mathf.Abs(testNode.y - currentNode.y) +
                        Mathf.Abs(testNode.z - currentNode.z) >= 2)
                        travelCost = 1.4f;
                    else
                        travelCost = 1.0f;
                    ...
                    else
                    {
                        ...
                        //FIXME_VAR_TYPE start= Time.realtimeSinceStartup;
                        while (i2 != 1)
                            if (Fcost(openList[i2], nodeList) <=
                                Fcost(openList[(int) Mathf.Floor((int) i2 / 2)], nodeList))
                            {
    ...
    {
        return Mathf.Abs(node.x - destinationNode.x) * cost + Mathf.Abs(node.y - destinationNode.y) * cost +
               Mathf.Abs(node.z - destinationNode.z) * cost;
    }

    public static float euclidianHeuristic(Node node, Node destinationNode, float cost)
    {
        return new Vector3(node.x - destinationNode.x, node.y - destinationNode.y, node.z - destinationNode.z)
            .magnitude;
    }
}